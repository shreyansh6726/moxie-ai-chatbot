      ...
      <div className="chat-card">
        ...
        <form onSubmit={handleSend} className="chat-input-form">
          ...
          <div className="input-wrapper">
            <input type="file" accept=".txt" ref={fileInputRef} style={{ display: 'none' }} onChange={(e) => {
              const file = e.target.files[0];
              if (file) {
                const reader = new FileReader();
                reader.onload = (ev) => setAttachedFile({ name: file.name, content: ev.target.result, size: (file.size / 1024).toFixed(1) + " KB" });
